    ...
    public function parse($data)
    {
        ...
        if (is_array($records) === true
            ...
            foreach ($records as $record) {
                ...
                foreach ($authors as $author => $method) {
                    if (isset($record[$author]) == true) {
                        foreach ($record[$author] as $value) {
                            $entry = $this->setPersons($entry, $method, $value);
                        }
                    }
                }
                ...
                //added
                if(isset($record['ID'])){
                    $entry->setCitationLabel($record['ID'][0]);
                }
                if(isset($record['CY'])){
                    $entry->setAddress($record['CY'][0]);
                }
...
die('aaa');
    ...
    private function getType(Entry $entry, $type)
    {
        switch ($type) {
            case 'CONF':
                $entry->getType()->setInproceedings();
                break;
            case 'MISC':
                $entry->getType()->setMISC();
                break;
            case 'INCOL':
                $entry->getType()->setIncollection();
                break;
            case 'ABST':
     ...
     * @return mixed
     */
    private function setPersons($entry, $method, $data){
        $values = explode(';',$data);
